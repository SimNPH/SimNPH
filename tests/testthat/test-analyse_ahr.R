test_that("analyse ahr works", {
  ...
  results4 <- analyse_ahr(max_time=50, type="gAHR")(condition, dat)

  expect_type(results1, "list")
  expect_s3_class(results1, NA)
  expect_named(results1, c("p", "AHR", "AHR_lower", "AHR_upper", "N_pat", "N_evt"))

  expect_type(results2, "list")
  expect_s3_class(results2, NA)
  expect_named(results2, c("p", "gAHR", "gAHR_lower", "gAHR_upper", "N_pat", "N_evt"))

  expect_type(results3, "list")
  expect_s3_class(results3, NA)
  expect_named(results3, c("p", "AHR", "AHR_lower", "AHR_upper", "N_pat", "N_evt"))

  expect_type(results4, "list")
  expect_s3_class(results4, NA)
  expect_named(results4, c("p", "gAHR", "gAHR_lower", "gAHR_upper", "N_pat", "N_evt"))
