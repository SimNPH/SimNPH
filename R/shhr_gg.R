...
shhr_gg <- function(A, B, main=NULL, sub=NULL, group_names=c("control", "treatment"), lab_time="Days", lab_group="Group"){
  ...
  gg_surv <- ggplot(plotdata, aes(x=t)) +
    geom_line(aes(y=surv_a, colour=group_names[1], lty=group_names[1])) +
    geom_line(aes(y=surv_b, colour=group_names[2], lty=group_names[2])) +
    labs(
      x=lab_time,
      y="Survival",
      colour=lab_group,
  ...
  gg_haz <- ggplot(plotdata, aes(x=t)) +
    geom_line(aes(y=haz_a, colour=group_names[1], lty=group_names[1])) +
    geom_line(aes(y=haz_b, colour=group_names[2], lty=group_names[2])) +
    labs(
      x=lab_time,
      y="Hazard",
      colour=lab_group,
