...
source("./R/analyse_ma.R")

# Script options/settings --------------------------------------------------------
N_sim <- 500 # number of simulations per scenario
...
save_folder <- file.path(
  #"..",
  "data",
  paste0("simulation_ext_data_ma_2_",
         Sys.info()["nodename"], "_",
         strftime(Sys.time(), "%Y-%m-%d_%H%M%S")))

sim_data_dir <- file.path("..","..","parametric_simulations","sim_data_3")

design_table <- file.path("data","parameters","ext_data_2023-05-21.csv")
...
Design <- read.table(
  design_table,
  sep=",", dec=".", header=TRUE)
Design <- Design[c(5,9,13,17,21,25,29,33,37,41,45),]
...
my_analyse <- list(
  ma = analyse_ma(alternative = "one.sided")
...
my_summarise <- create_summarise_function(
  ma = summarise_estimator(est=med_diff_est, real=med_diff,lower=med_diff_lower,upper=med_diff_upper,null=0,name="med_time_diff"),
