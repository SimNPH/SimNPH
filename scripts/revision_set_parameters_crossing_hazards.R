...
design <- design |> mutate(exp_fup  = rqm(hazard_trt_after,.2)+recruitment/30.4375)
...
design <- purrr::map_dfr(1:nrow(design),function(n){
  data <- design[n,]
  true_summary_statistics_crossing_hazards(data,
...
filename <- paste0("data/parameters/revision_crossing_hazards_", format(Sys.Date(), "%Y-%m-%d"), ".csv")
