...
generate_read <- function(condition, fixed_objects=NULL){
  ...
  filename <- paste0(
    ...
    "../../parametric_simulations/test_sim_package/testdata/scenario_",
...
save_folder <- paste0(paste0("data/simulation_parametric_with_external_data_", Sys.info()["nodename"], "_", strftime(Sys.time(), "%Y-%m-%d_%H%M%S")))
...
results$weibull.rejection_0.05
results$logrank.rejection_0.05
