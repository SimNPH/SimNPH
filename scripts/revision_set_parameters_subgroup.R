...
design <- design |> mutate(exp_fup  = rqm(hazard_subgroup,.2)+recruitment/30.4375)
...
design <- purrr::map_dfr(1:nrow(design),function(n){
  data <- design[n,]
  true_summary_statistics_subgroup(data,
...
filename <- paste0("data/parameters/revision_subgroup_", format(Sys.Date(), "%Y-%m-%d"), ".csv")
