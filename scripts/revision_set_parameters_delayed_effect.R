...
assumptions <- expand.grid(
  ...
  delay = m2d(seq(0,9, by=3)),#m2d(seq(0,9, by=2)),
  effect_size_ph = c(0, 0.8)#c(0, 0.5, 0.8, 0.9)
)
...
design <- purrr::map_dfr(1:nrow(design),function(n){
  data <- design[n,]
  true_summary_statistics_delayed_effect(data,
...
filename <- paste0("data/parameters/revision_delayed_effect_", format(Sys.Date(), "%Y-%m-%d"), ".csv")
